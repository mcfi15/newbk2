...
$credit = $user_home->runQuery("SELECT * FROM transfer ORDER BY id DESC LIMIT 200");
$credit->execute();
...
$upStatus = $user_home->runQuery("UPDATE transfer SET status = '$status' WHERE id = '$id'");
$upStatus->execute();
$_SESSION['msg'] = "
                    <div class='alert alert-info'>
                        <button class='close' data-dismiss='alert'>&times;</button>
                        <strong>Success!</strong> Transaction Has Been Updated Successfully!
                   
                    </div>
                    ";
$user_home->redirect("credit_debit_list");
...
<!--[if IE 9 ]><html class="ie9"><![endif]-->
    <head>
		...
		<title><?php echo $sitename; ?> | Admin - Credit/Debit History</title>
	...
	<?php while($row = $credit->fetch(PDO::FETCH_ASSOC)){?>
                                <tr>
                                    ...
                                    <td><?php echo date('jS \of F y h:i:s A', $row['dot']); ?></td>
                                    <td>
